...
def detect_communities(
    ...
    """
    This function generates a dataframe with the original bioassay readout information for each community.
    If a community contains 3 or more samples, the order of the shown samples is determined by the samples
    correlation to each other. This shall allow easier readability of the heat map. The reordering is based
    on hierarchical clustering, using the complete linkage method. Input data might be standardized (mean=0, sd=1)
    ...
    :param standardization: Boolean to indicate whether bioassay readout information shall be standardized before
                            clustering.
    ...
    for community in range(community_count + 1):
        ...
        if assay_df_subset.shape[0] >= 3:
            try:
                ...
                logger.warn(
                    f"Unable to optimize assay order using heirarchical clustering on community {community}"
                )
